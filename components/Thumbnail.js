...
const Thumbnail = forwardRef(({ result }, ref) => {
    ...
    return (
        ...
        className="group cursor-pointer p-2 transition duration-200 ease-in transform sm:hover:scale-105 hover:z-50">
            ...
            />
            <div className="p-2">
                <h2 className="my-1 text-xl text-white transition-all duration-100 ease-in-out group-hover:font-bold">{result.title || result.original_name}</h2>

                <p className="truncate max-w-md" >{result.overview}</p>

                <p className="flex items-center  opacity-0 transition group-hover:opacity-100">
